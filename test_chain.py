...
SEG_INDEX = 2      # 8-digit 7-segment panel
...
def run_chain_test() -> None:
    ...
    try:
        ...
        print("Opening chain on /dev/spidev0.0 (3 modules) ...")
        chain = Max7219Chain(bus=0, device=0, num_modules=3, max_speed_hz=1000000)
        print("  OK")
        ...
        for i, word in enumerate(words):
            ...
            seg8.set_text(word)
            print(f"  matrix1='{chr(ord('A') + i)}'  matrix2='{chr(ord('A') + i + 1)}'  7seg='{word}'")
            time.sleep(1.0)
